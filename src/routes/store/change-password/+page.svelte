...
<div class="flex min-h-svh items-center justify-center bg-background p-4">
	<Card.Root class="w-full max-w-md">
		<Card.Header>
			...
			<Card.Description>{t(locale, 'storeApp.changePassword.description')}</Card.Description>
		</Card.Header>
		<Card.Content>
			<form method="POST" id="store-change-password-form" class="grid gap-4">
				{#if form?.message}
		...
		<Card.Footer>
			<Button type="submit" form="store-change-password-form" class="w-full">
